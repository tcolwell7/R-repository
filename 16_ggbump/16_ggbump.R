...
# add flags
#' https://github.com/davidsjoberg/ggbump/wiki/geom_bump-with-flags
#' simple and easy to add flag using the geom_flag utilising the iso code in lower case

bump_final <-
  bump + 
    geom_flag(
     data = df %>% filter(year == min(year)), 
       aes(x=2000+.3,country = tolower(iso2)),
       size = 8
      )
  

ggsave("png/my_first_bump_plot.png",bump_final,width=10,height=5)