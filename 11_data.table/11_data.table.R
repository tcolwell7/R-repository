...
tic()
eu_pref_imports <- fread("data/prefs2019.dat") # uploads .dat files and identfies the delimiter automatically. 
toc()


#' data.table can easily upload a 10mil row into R environment within a few seconds. 
#' This is quick. However an additional package vroom enables even quicker loading for reference!

# library(vroom)
tic()
df <- vroom::vroom("data/prefs2019.dat",delim=",")
toc()

#' *around half the time as fread for the .dat file* 
...
# simple example creating a variable based on trade value using nested ifelse

tic()
df <- df[, val := fifelse(value_in_euro <= 10000, "Small",
                  fifelse(value_in_euro <= 100000, "Medium", "Large"))
         ]
...
# normal ifelse
tic()
df <- df[, val := ifelse(value_in_euro <= 10000, "Small",
                  ifelse(value_in_euro <= 100000, "Medium", "Large"))
         ]
toc()

#'*fifelse is atleat 20x faster*
#'
...
tic()
df2 <- df[, val := fifelse(value_in_euro <= 10000, "Small",
  ...
  df[, 
     val := 
       fcase(
         value_in_euro <= 10000, "Small",
         value_in_euro <= 100000, "Medium",
         value_in_euro <= 1000000, "Medium-Large",
         value_in_euro <= 10000000,"Large",
         value_in_euro >  10000000, "Very-Large"
       )
  ]
toc()


#' *fifelse is slightly faster for a 3 condition statement* 
#' *when increased to 5 conditions fcase is faster*
#' *also fcase syntax looks cleaner and simpler to write*
#' *recommend face for multiple conditions columns!* 
...
#' pivoting data from long to wide or wide to long is a must when working with data. 
#' In tidyverse we have the easy to use user-friendly pivot_wider and pivot_longer. 
#' In data.table there are custom built dcast and melt to serve the same purpose, but faster!

df <- data.table::fread("data/prefs2019.dat") %>% clean_names()

# simplified aggregated dt in long format:
df_agg <- df[,.(value = sum(value_in_euro)),by=.(partner_iso,product,period)]

### dcast (long/wide) ------------------------------------------

# the EU preferential imports data is in a long data format. 
# the data is broken down by country/partner country and shows imports
# across each commodity code for each month. 
# we can convert this to wide - so the months are across as columns. 
...
df_wide <- 
  dcast.data.table(
    df_agg,
    formula = partner_iso + product ~period, # name columns to remain and those to pivot: ~
    fun.aggregate = sum # if data is dis-aggregated what function to apply
  )
toc()

# 295649
# more complicated:
# select columns from larger dt:
df_l = 
  df[, .(declarant_iso,partner_iso,product,stat_regime,eligibility,use,period,value_in_euro)]

# we want to pivot the period column only - so each month is a column
tic()
df_wide2 <- 
  dcast.data.table(
    df_l,
    formula = declarant_iso+partner_iso+product+stat_regime+eligibility+use  ~period, # name columns to remain and those to pivot: ~
    fun.aggregate = sum,# if data is dis-aggregated what function to apply
    value.var = "value_in_euro"
    )
toc()

# alternatively you can denote all columns to not pivt using ' ... '

tic()
df_wide2 <- 
  dcast.data.table(
    df_l,
    formula = ... ~ period, # name columns to remain and those to pivot: ~
    fun.aggregate = sum,# if data is dis-aggregated what function to apply
    value.var = "value_in_euro"
  )
toc()
...
# example using df_wide from dcast example
# to convert wide to long using melt:
df_wide <- df_wide %>% clean_names()

df_melt <- 
    melt.data.table(
      data = df_wide,
      id.vars = c("partner_iso","product"),
      measure.vars = 3:15, # name column indexes
      value.name = "value" # name column name for values
    )
...
df_melt <- 
  melt.data.table(
    data = df_wide,
    id.vars = c("partner_iso","product"),
    measure.vars = c("x201901","x201902","x201903","x201904"), # name column by name. If you name 4 columns it only pivots 4 and revmoes the rest. 
    value.name = "imports",
    variable.name = "month" # name melted variable column name
  )
