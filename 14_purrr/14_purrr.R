...
# you can easily utilise map
# o create multiple data frames and seamlessly export
# them into your global environment
# the following example uploads data from a workbook
# sores the dfs in a list and then export creating individual dfs


# define sheet names to map
sheetname = c("1 Total Trade by Country - A",
              "2 Total Trade by Country - Q",
              "3 TiG by Country - A",
              "4 TiG by Country - Q",
              "5 TiS by Country - A",
              "6 TiS by Country - Q")

# specify names to label each dataset
df_names <- c("trade_annual",
              "trade_quarterly",
              "goods_annual",
              "goods_quarterly",
              "services_annual",
...
ons_data <- 
  purrr::map( # mapping iterates an input through a function to product individual outpus stored in a list
    sheetname,
    function(x)
      read_excel( 
        "data/ons_trade.xlsx", # load in data
        sheet = x, # map through sheet names
        skip = 3 # skip 3 rows
      )
  ) %>% # pipe connects functions to another in one query
  purrr::set_names(.,df_names) %>% # set df names
  list2env(.,envir=.GlobalEnv) # export items from list into environment
